...
void load(KVEC *kvr, int num, bool seq, int tid) {
    RandomGenerator gen;
    Random rand(0);
...
void update(KVEC *kvr, int num, bool seq, int tid) {
  RandomGenerator gen;
  Random rand(0);
  kvr_key *keys = new kvr_key[num];
  kvr_value *vals = new kvr_value[num];
...
int main() {
    ...
    //KVEC kvr(k, r, 1, slab_list, ssds);
    KVEC kvr(k, r, 1, slab_list, kvs_conts, Mem);
